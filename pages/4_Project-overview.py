    ...
    # st.write(columnames)
        ...
        with st.container():
            ...
            with col1:
                ...
                df1['Total_cost_display'] = df1['Total_cost'].astype(str) + " €"
                figrate2 = px.bar(df1, y='Total_cost', x='alias', text=df1['Total_cost'].astype(str) + " €",title="Project Manager: "+str(selected_option2)+" - Cost of project per user")
...
# Display the chart
        ...
        sql6 = f""" SELECT kimai2_users.alias,kimai2_projects.name,kimai2_timesheet.start_time,kimai2_timesheet.duration,kimai2_user_preferences.value 
    FROM `kimai2_timesheet`
    INNER JOIN `kimai2_users` ON kimai2_users.id=kimai2_timesheet.user
    INNER JOIN `kimai2_user_preferences` ON kimai2_users.id=kimai2_user_preferences.user_id
    Inner JOIN `kimai2_projects` ON kimai2_projects.id=kimai2_timesheet.project_id
    ...
    # Apply the formatting function to the 'Year' column
        ...
        df5['Total_cost'] = df5['duration'] * df5['value']
